...
- **`room_rate_calculator`** - Code for the application's Lambda function.
...
```bash
test-sam$ make run
test-sam$ curl http://localhost:3000/hello
```
...
```bash
test-sam$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
test-sam$ aws cloudformation delete-stack --stack-name test-sam
```
...
```bash
test-sam$ pipenv lock -r > hello_world/requirements.txt
```
